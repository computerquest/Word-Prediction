...
void PredictionEngine::findType() {
	LinkedList<vector<double>, vector<double>> masterPrint;
	vector<vector<double>> allDesired;
	...
	Neuron nounI(0);
	{
		vector<double> tempDesired;
		tempDesired.push_back(0);
		tempDesired.push_back(1);
		tempDesired.push_back(0);

		neurons.push_back(nounI);
		allDesired.push_back(tempDesired);
	}
	Neuron verbI(0);
	{
		vector<double> tempDesired;
	...
	Neuron conjunctionI(0); // have list
	{
		...
		neurons.push_back(conjunctionI);
		allDesired.push_back(tempDesired);
	}
	...
	Neuron prepositionI(0); //have list
	{
		...
		neurons.push_back(prepositionI);
		allDesired.push_back(tempDesired);
	}
	...
	Neuron nextAdjective(0);
	{
		...
		neurons.push_back(nextAdjective);
		allDesired.push_back(tempDesired);
	}
	...
	Neuron nextPreposition(0);
	{
		...
		neurons.push_back(nextPreposition);
		allDesired.push_back(tempDesired);
	}
	Neuron nextConjunction(0);
	{
		...
		neurons.push_back(nextConjunction);
		allDesired.push_back(tempDesired);
	}

	Neuron nounNG(0);//double
	{
		vector<double> tempDesired;
		tempDesired.push_back(1);
		tempDesired.push_back(0);
		...
		neurons.push_back(nounNG);
		allDesired.push_back(tempDesired);
	}
	Neuron verbNG(0);//double
	{
		vector<double> tempDesired;
		tempDesired.push_back(0);
		tempDesired.push_back(1);
		...
		neurons.push_back(verbNG);
		allDesired.push_back(tempDesired);
	}
	Neuron adjectiveNG(0);//double
	{
		vector<double> tempDesired;
		tempDesired.push_back(0);
		tempDesired.push_back(0);
	...
	Neuron bestAdjectiveN(0);
	{
		...
		neurons.push_back(bestAdjectiveN);
		allDesired.push_back(tempDesired);
	}
	...
	Neuron bestAdjectiveWn(0);
	{
		...
		neurons.push_back(bestAdjectiveWn);
		allDesired.push_back(tempDesired);
	}

	Neuron nounN(0);//double
	{
		vector<double> tempDesired;
		tempDesired.push_back(1);
		tempDesired.push_back(0);
		...
		neurons.push_back(nounN);
		allDesired.push_back(tempDesired);
	}
	Neuron verbN(0);//double
	{
		vector<double> tempDesired;
		tempDesired.push_back(0);
		tempDesired.push_back(1);
		...
		neurons.push_back(verbN);
		allDesired.push_back(tempDesired);
	}
	Neuron adjectiveN(0);//double
	{
		vector<double> tempDesired;
		tempDesired.push_back(0);
		tempDesired.push_back(0);
		...
		neurons.push_back(adjectiveN);
		allDesired.push_back(tempDesired);
	}

	//training = createTrain(neurons, allDesired);

	for (int z = 0; z < examples.size(); z++) {
		std::cout << examples.at(z) << endl;
		string phrase = examples.at(z);

		vector<string> wordS = breakDownV(examples.at(z));

		for (int i = 0; i < wordS.size(); i++) {
			vector<double> blueprint;
			vector<POS> wtype = master->findAllWordType(examples.at(z));
			SStructure structure;

			structure = master->findPartailStructure(wtype, i);
			vector<SStructure> possibilities = master->matchPossibleSS(wtype);
			if (wtype.at(i) != POS::Conjuction && wtype.at(i) != POS::Article && wtype.at(i) != POS::Preposition) {
				//inputs
				if (i > 0) {
				...
				//last structure stuff
				{
					if (lastPhrase != "") {
						vector<string> lastPhraseS = breakDownV(lastPhrase);
						vector<POS> lastPOS;

						for (int a = 0; a < lastPhraseS.size(); a++) {
							lastPOS.push_back(master->findWord(lastPhraseS.at(a)).type);
						}

						NGram<SStructure> lastStruct = master->findNGramS(lastPOS);
						int index = -1;
						...
						if (index != -1) {
							SStructure structure = lastStruct.content.getValueI(index);

							if (structure.component.size() > i + 1) {
								POS type = structure.component.at(i);
				...
				//goes through possibility sstructure and finds the amount supported for each type
				{
					double nTotal = 0;
					double vTotal = 0;
					double aTotal = 0;
					...
					for (int a = 0; a < possibilities.size(); a++) {
						if (possibilities.at(a).component.size() > i + 1) {
				...
				//find anything possibility matching the last ngrams content
				...
				double nounV = verbI.value + adjectiveI.value + conjunctionI.value + articleI.value + prepositionI.value + predictN.value + endN.value
					+ nextVerb.value + nextAdjective.value + nextArticle.value + nextPreposition.value + nextConjunction.value;

				double verbV = nounI.value + verbI.value + adjectiveI.value + conjunctionI.value + articleI.value + prepositionI.value + predictN.value + endVerb.value
					+ (verbI.value == 0 ? nextVerb.value : 0) + nextNoun.value + nextAdjective.value + nextArticle.value + nextPreposition.value + nextConjunction.value;

				double adjectiveV = nounI.value + verbI.value + adjectiveI.value + conjunctionI.value + articleI.value + prepositionI.value + predictN.value + endA.value
					+ nextVerb.value + nextNoun.value + nextAdjective.value + nextArticle.value + nextPreposition.value + nextConjunction.value;

				double total = nounV + verbV + adjectiveV;

				if (wtype.at(i) == POS::Noun) {
					desired.push_back(1);
					desired.push_back((verbV / total) - 1);
					desired.push_back((adjectiveV / total) - 1);
				}
				else if (wtype.at(i) == POS::Verb) {
					desired.push_back((nounV / total) - 1);
					desired.push_back(1);
					desired.push_back((adjectiveV / total) - 1);
				}
				else if (wtype.at(i) == POS::Adjective) {
					desired.push_back((nounV / total) - 1);
					desired.push_back((verbV / total) - 1);
					desired.push_back(1);
				}

				training.add(blueprint, desired);
			}
			else {
				continue;
			}
		...
		lastPhrase = examples.at(z);
	}
	nn.autoPrune(training, false);
	save();
} //can call find type deployment from method and add loop
  /*POS PredictionEngine::findTypeDeployment(string phrase) {
  vector<Word> word;
  ...
  vector<double> blueprint;
  vector<POS> wtype = master->findAllWordType(phrase);
  SStructure structure;
  int i = 0;

  for (int a = 0; a < wtype.size(); a++) {
  if (wtype.at(a) == POS::Unknown) {
  i = a;
  }
  }
  ...
  } //that is the regular findTypeDeployment
  POS PredictionEngine::findTypeDeployment(string phrase, int unknownWord) {
  vector<Word> word;
  vector<double> masterPrint;
  vector<string> wordS = breakDownV(phrase);
  int unknown = 0;
...
POS PredictionEngine::findTypeDeployment(vector<POS> wtype, string phrase, int i) {
	...
	if (wtype.at(i) != POS::Conjuction && wtype.at(i) != POS::Article && wtype.at(i) != POS::Preposition) {
		//inputs nounI
		if (i > 0 && wtype.at(i - 1) != POS::Unknown) {
